...
class PomPomNN(nn.Module):
    ...
    def forward(self, x):
        ...
        policy_output = self.policy_fc(policy)  # Linear layer output (batch_size, 81)
        policy_output = F.softmax(policy_output, dim=1)  # Softmax normalization, obtain probability distribution
